...
import { allHistoricalEvents } from '../backend/allHistoricalEvents';
import { historicalEvents } from '../backend/historicalEvents';
import { historicalRulers } from '../backend/historicalRulers';
import { indianHistoricalEvents } from '../backend/indianHistoricalEvents';
import { sixteenthCenturyEvents } from '../backend/sixteenthCenturyEvents';
import { nineteenthTwentiethCenturyEvents } from '../backend/nineteenthTwentiethCenturyEvents';
...
function HistoricalEventsApp() {
  ...
  const allHistoricalData = [
    ...
    ...Object.entries(historicalEvents).map(([key, event]) => ({
      id: key,
      ...event,
      isWorldEvent: true // Mark world events
    })),
  ...
  const categories = [
    ...
    { id: 'colonial', name: 'Colonial Era', icon: <Calendar size={18} /> },
    { id: 'modern', name: 'Modern Era', icon: <Calendar size={18} /> }
  ];
  
  // Categorize events
  const categorizeEvent = (eventKey) => {
    if (eventKey?.includes('ancient')) return 'ancient';
    if (eventKey?.includes('medieval')) return 'medieval';
    if (eventKey?.includes('colonial')) return 'colonial';
    if (eventKey?.includes('modern')) return 'modern';
    return 'all';
  };
  ...
  useEffect(() => {
    ...
    // Add random events
    while (randomEvents.length < Math.min(6, allHistoricalData.length)) {
      const randomIndex = Math.floor(Math.random() * allHistoricalData.length);
      
      if (!usedIndices.has(randomIndex)) {
        usedIndices.add(randomIndex);
        const event = allHistoricalData[randomIndex];
        const category = categorizeEvent(event.id);
        
        randomEvents.push({
          key: event.id,
          title: event.title || event.name,
          date: event.year || event.date || event.period,
          image: event.image,
          category
        });
      }
  ...
  const handleSearch = () => {
    ...
    setTimeout(() => {
      ...
      // Search through all historical data with improved matching
      const searchResults = allHistoricalData
        .map(event => {
      ...
      if (searchResults.length > 0) {
        const selectedEvent = searchResults[0].event;
        setEvent({
          ...selectedEvent,
          image: selectedEvent.images?.[0]?.image || selectedEvent.image
        });
        addToViewHistory({
          key: selectedEvent.id,
          title: selectedEvent.title || selectedEvent.name,
          date: selectedEvent.year || selectedEvent.date || selectedEvent.period,
          image: selectedEvent.images?.[0]?.image || selectedEvent.image,
          category: categorizeEvent(selectedEvent.id)
        });

        // If there are multiple good matches, show suggestions
        if (searchResults.length > 1 && searchResults[1].matchScore > 50) {
          const suggestions = searchResults.slice(1, 4).map(result => result.event);
          setSuggestedEvents(suggestions.map(event => ({
            key: event.id,
            title: event.title || event.name,
            date: event.year || event.date || event.period,
            image: event.images?.[0]?.image || event.image,
            category: categorizeEvent(event.id)
          })));
        }
      } else {
        // Provide helpful suggestions based on the search term
        let suggestedSearchTerms = '';
        if (normalizedSearch.includes('ancient') || normalizedSearch.includes('old')) {
          suggestedSearchTerms = '"indus valley", "mauryan empire", "ashoka", "chandragupta maurya"';
        } else if (normalizedSearch.includes('medieval') || normalizedSearch.includes('middle')) {
          suggestedSearchTerms = '"delhi sultanate", "vijayanagara empire", "mughal empire", "maratha empire"';
        } else if (normalizedSearch.includes('colonial') || normalizedSearch.includes('british')) {
          suggestedSearchTerms = '"british east india company", "indian national congress", "gandhi", "tipu sultan"';
        } else if (normalizedSearch.includes('modern') || normalizedSearch.includes('recent')) {
          suggestedSearchTerms = '"indian independence", "economic liberalization", "nehru", "world war 2"';
        } else {
          suggestedSearchTerms = '"indus valley", "mauryan empire", "vijayanagara empire", "mughal empire", "maratha empire", "indian independence"';
        }
        
        setError(`No exact matches found. Try searching for ${suggestedSearchTerms} or refine your search terms.`);
      }
  ...
  const handleSuggestedEventClick = (eventKey) => {
    ...
    setTimeout(() => {
      const selectedEvent = allHistoricalData.find(event => event.id === eventKey);
      if (selectedEvent) {
        ...
        setError('Event not found. Please try searching again.');
      }
      setLoading(false);
    }, 400);
  };
  ...
  const addToViewHistory = (eventData) => {
    if (!viewHistory.some(item => item.key === eventData.key)) {
      setViewHistory(prev => [{ ...eventData }, ...prev].slice(0, 5));
    }
  };
        ...
        >
          <div className="event-header">
            <div className="event-title-section">
              ...
              <p className="event-date"><Clock size={18} className="icon" /> {event.year}</p>
              <div className="event-category-tag">
                {event.id.startsWith('ancient') ? (
                  <span><BookOpen size={14} className="mr-1" /> Ancient Era</span>
                ) : event.id.startsWith('medieval') ? (
                  <span><Award size={14} className="mr-1" /> Medieval Era</span>
                ) : event.id.startsWith('colonial') ? (
                  <span><Calendar size={14} className="mr-1" /> Colonial Era</span>
                ) : event.id.startsWith('modern') ? (
                  <span><Calendar size={14} className="mr-1" /> Modern Era</span>
                ) : event.id.startsWith('maratha') ? (
                  <span><Award size={14} className="mr-1" /> Maratha Era</span>
                ) : (
                  <span><Globe size={14} className="mr-1" /> All Events</span>
                )}
              </div>
            </div>
          ...
          <div className="event-content" ref={eventContentRef}>
            ...
            {event.images && event.images.length > 0 && (
              <motion.div 
                ...
                transition={{ duration: 0.5, delay: 0.4 }}
              >
                <h3 className="section-title"><Image size={22} className="icon" /> Additional Images</h3>
                <PhotoProvider>
                  <div className="images-grid">
            ...
            >
              <h3 className="section-title"><Info size={22} className="icon" /> Description</h3>
            ...
            {event.details && event.details.length > 0 && (
              ...
              >
                <h3 className="section-title"><BookOpen size={22} className="icon" /> Details</h3>
                <ul>
            ...
            {event.impact && (
              <motion.div 
                ...
                transition={{ duration: 0.5, delay: 0.4 }}
              >
                <h3 className="section-title"><Award size={22} className="icon" /> Impact</h3>
                <p>{event.impact}</p>
              </motion.div>
            )}
            ...
            {event.videoLinks && event.videoLinks.length > 0 && (
              ...
              >
                <h3 className="section-title"><Youtube size={22} className="icon" /> Related Videos</h3>
